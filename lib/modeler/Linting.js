...
export default class Linting {
  constructor(canvas, config, elementRegistry, eventBus, injector, lintingAnnotations, selection) {
    ...
    // optional: linting may run without a properties panel
    this._propertiesPanel = injector.get('propertiesPanel', false);
  ...
  showError(report) {
    ...
    const entryIds = getReportEntryIds(report, selectableElement, this._propertiesPanel);
  ...
  _update() {
    ...
    this._eventBus.fire('propertiesPanel.setErrors', {
      errors: getErrors(this._reports, selectedElement, this._propertiesPanel)
    });
  }
