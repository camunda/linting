...
import { getEntryIds, getErrors } from '../utils/properties-panel';
...
export default class Linting {
  ...
  showError(report) {
    ...
    const entryIds = getEntryIds(report, selectableElement && selectableElement.id);
  ...
  setErrors(reports) {
    ...
    // let render modules (e.g. lint-autofix) react to fresh lint results
    this._eventBus.fire('linting.reportsChanged', { reports });
