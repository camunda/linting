...
import { getDiagnostics, getReportEntryIds } from '../utils/properties-panel';
...
export default class Linting {
  ...
  _update() {
    ...
    // set properties panel diagnostics
    const selectedElement = this._getSelectedElement();

    this._eventBus.fire('propertiesPanel.setDiagnostics', {
      diagnostics: getDiagnostics(this._reports, selectedElement, (element, path) => this._resolveEntryId(element, path))
    });
  }
