...
export function getErrorMessage(report, executionPlatform, executionPlatformVersion, modeler = 'desktop') {
  ...
  if (type === ERROR_TYPES.SECRET_EXPRESSION_FORMAT_DEPRECATED) {
    return getSecretExpressionFormatDeprecatedErrorMessage(report, executionPlatformVersion);
  }

  if (type === ERROR_TYPES.SECRET_EXPRESSION_FORMAT_LEGACY) {
    return getSecretExpressionFormatLegacyErrorMessage(report);
  }
...
function getSecretExpressionFormatDeprecatedErrorMessage(report, executionPlatformVersion) {
  const { data } = report;

  const { property, allowedVersion } = data;

  if (allowedVersion && executionPlatformVersion && greaterOrEqual(executionPlatformVersion, allowedVersion)) {
    return `Property <${ property }> uses deprecated secret expression format secrets.SECRET, migrate to camunda.secrets format`;
  }

  return `Property <${ property }> uses deprecated secret expression format secrets.SECRET, use {{secrets.SECRET}} instead`;
}

function getSecretExpressionFormatLegacyErrorMessage(report) {
  const { data } = report;

  const { property } = data;

  return `Property <${ property }> uses legacy secret expression format {{secrets.SECRET}}, migrate to camunda.secrets format`;
}
