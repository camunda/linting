...
function getExtensionElementNotAllowedErrorMessage(report, executionPlatformVersion) {
  ...
  if (is(node, 'bpmn:BusinessRuleTask') && is(extensionElement, 'zeebe:CalledDecision')) {
    return `A <Business Rule Task> with <Implementation: DMN decision> is not supported by ${ getExecutionPlatformLabel('Camunda Cloud', executionPlatformVersion) }`;
  }

  if (is(extensionElement, 'zeebe:Properties')) {
    return `${ getIndefiniteArticle(typeString) } <${ typeString }> with <Extension properties> is not supported by ${ getExecutionPlatformLabel('Camunda Cloud', executionPlatformVersion) }`;
  }
...
function getPropertyNotAllowedErrorMessage(report, executionPlatformVersion, modeler = 'desktop') {
  ...
  if (property === 'modelerTemplate') {
    if (modeler === 'desktop') {
      return `${ getIndefiniteArticle(typeString) } <Template ${ typeString }> is not supported by ${ getExecutionPlatformLabel('Camunda Cloud', executionPlatformVersion) }`;
    } else if (modeler === 'web') {
      return `${ getIndefiniteArticle(typeString) } <Connector ${ typeString }> is not supported by ${ getExecutionPlatformLabel('Camunda Cloud', executionPlatformVersion) }`;
    }
  }
