...
export function getErrorMessage(report, executionPlatformLabel, modeler = 'desktop') {
  ...
  if (type === ERROR_TYPES.EXPRESSION_REQUIRED) {
    return getExpressionRequiredErrorMessage(report, executionPlatformLabel);
  }

  if (type === ERROR_TYPES.EXPRESSION_VALUE_NOT_ALLOWED) {
    return getExpressionValueNotAllowedErrorMessage(report, executionPlatformLabel);
  }
...
function getExpressionRequiredErrorMessage(report, executionPlatformLabel) {
  const {
    data
  } = report;
...
function getExpressionValueNotAllowedErrorMessage(report, executionPlatformLabel) {
  const {
    data
  } = report;
