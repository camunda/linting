...
// maps a bpmnlint report category to the properties panel's diagnostic
// severity; a category with no entry here (e.g. `rule-error`, which means the
// rule itself threw rather than found a modeling problem) is not surfaced as
// a diagnostic
...
/**
 * Get diagnostics for a given element, keyed by entry id.
 *
 ...
 *
 * @returns {Object<string, Array<{ severity: string, message: string }>>}
 */
export function getDiagnostics(reports, element, resolveEntryId) {
  return reports.reduce((diagnostics, report) => {
    const severity = SEVERITY_BY_CATEGORY[ report.category ];
    ...
    // an entry can carry several diagnostics (e.g. more than one report
    // targeting the same field); the panel ranks them by severity itself
    return {
      ...diagnostics,
      ...ids.reduce((entryDiagnostics, id) => {
        return {
          ...entryDiagnostics,
          [ id ]: [ ...(diagnostics[ id ] || []), { severity, message } ]
        };
      }, {})
    };
  }, {});
...
export function getEntryIds(report, prefixId) {
  ...
  // (e.g., bpmnlint/conditional-event, camunda-platform/history-time-to-live)
...
export function getErrorMessage(id, report) {
  ...
  if (type === ERROR_TYPES.SECRET_REFERENCE_STRING_LITERAL_NOT_ALLOWED) {
    return 'Secret reference must be an expression, not a string literal.';
  }

  if (type === ERROR_TYPES.SECRET_REFERENCE_INSIDE_LIST_NOT_ALLOWED) {
    return 'Secret reference must not be nested in a list.';
  }

  if (type === ERROR_TYPES.SECRET_REFERENCE_INSIDE_IF_BRANCH_CONTEXT_NOT_ALLOWED) {
    return 'Secret reference must not be nested in an if branch.';
  }
