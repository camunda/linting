...
// A `fromAi()` finding (agent-fromai-contract) validates a single call inside a
// potentially larger FEEL expression, e.g. `="prefix " + fromAi(toolCall.query)`.
// In the properties panel the message renders next to that field, so:
// - the offending token stays (`toolCall.query`, the wrong function name, ...),
//   because it pinpoints which call in the expression is at fault, but
// - the trailing "how to fix" / "why" guidance is dropped, since it only
//   restates what is already obvious from the field being edited.
const AGENT_FEEL_TYPES = [
  ERROR_TYPES.AGENT_FEEL_FUNCTION_NAME_INVALID,
  ERROR_TYPES.AGENT_FEEL_KEY_MISSING,
  ERROR_TYPES.AGENT_FEEL_KEY_TYPE_INVALID,
  ERROR_TYPES.AGENT_FEEL_KEY_PREFIX_MISSING,
  ERROR_TYPES.AGENT_FEEL_KEY_SEGMENTS_INVALID,
  ERROR_TYPES.AGENT_FEEL_KEY_DUPLICATE,
  ERROR_TYPES.AGENT_FEEL_DESCRIPTION_TYPE_INVALID,
  ERROR_TYPES.AGENT_FEEL_WRONG_CONTEXT,
  ERROR_TYPES.AGENT_FEEL_NON_ENTRY_ELEMENT
];

const AGENT_FEEL_REDUNDANT_CLAUSES = [
  [ ' Declare it once and reference it directly elsewhere.', '' ],
  [ ' Define it in an input mapping on the tool\'s entry element.', '' ],
  [ ' Define it there and read the toolCall variable directly.', '' ],
  [ ' The connector requires a plain reference regardless of which branch would apply at runtime.', '' ],
  [ ': a quoted string describing what the agent should provide.', '.' ]
];
...
export function getErrorMessage(id, report) {
  ...
  // Shown next to the offending field, so trim the verbose `fromAi()` message
  // down to the part that pinpoints the fault, keeping the offending token.
  // See `agent-fromai-contract`.
  if (AGENT_FEEL_TYPES.includes(type)) {
    return simplifyAgentFeelMessage(message);
  }
...
/**
 * Trim a verbose `fromAi()` finding message to what makes sense next to the
 * offending field: keep the sentence that names the offending token, drop the
 * trailing "how to fix" / "why" guidance.
 *
 * @param {string} message
 *
 * @returns {string}
 */
function simplifyAgentFeelMessage(message) {
  if (!isString(message)) {
    return message;
  }

  return AGENT_FEEL_REDUNDANT_CLAUSES.reduce(
    (result, [ clause, replacement ]) => result.replace(clause, replacement),
    message
  );
}
