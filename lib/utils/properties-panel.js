...
export function getEntryIds(report, prefixId) {
  ...
  // Agent fromAi() context reports point at the formal expression itself,
  // without the generic validation metadata above. The sequence-flow panel
  // renders that expression through the same conditionExpression entry.
  if (path && path.length === 1 && path[ 0 ] === 'conditionExpression') {
    return [ 'conditionExpression' ];
  }
  ...
  // fromAi() contract errors (agent-fromai-contract rule) report against the
  // violated FEEL expression, not a moddle property, so they never set
  // data.node/data.property and the LIST_PROPERTIES match above can't apply.
  // Resolve the same entry id from the path alone: it still ends in
  // [...,'inputParameters'|'outputParameters', index, 'source'|'target'].
  if (path && path.length >= 3 && [ 'source', 'target' ].includes(path[ path.length - 1 ])) {
    const listProperty = path[ path.length - 3 ];
    const index = path[ path.length - 2 ];
    const property = path[ path.length - 1 ];

    if (listProperty === 'inputParameters') {
      return [ `${ id }-input-${ index }-${ property }` ];
    }

    if (listProperty === 'outputParameters') {
      return [ `${ id }-output-${ index }-${ property }` ];
    }
  }
